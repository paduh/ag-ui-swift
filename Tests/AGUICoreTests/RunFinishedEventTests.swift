...
final class RunFinishedEventTests: XCTestCase {
    ...
    func testEventTypeProperty() {
        // Given
        let event = RunFinishedEvent(threadId: "thread-123", runId: "run-456")
    ...
    func testEncoding() throws {
        ...
        XCTAssertEqual(json?["type"] as? String, "RUN_FINISHED")
    ...
    func testDecodingFailsWithWrongType() {
        ...
        // When & Then
        XCTAssertThrowsError(try decoder.decode(RunFinishedEvent.self, from: data)) { error in
    ...
    func testDecodingFailsWithMissingRequiredFields() {
        ...
        do {
            ...
            let data = try JSONSerialization.data(withJSONObject: json)
            let decoder = JSONDecoder()
            _ = try decoder.decode(RunFinishedEvent.self, from: data)
    ...
    func testRoundTripEncodingDecoding() throws {
        // Given
        let originalEvent = RunFinishedEvent(
            threadId: "thread-123",
            runId: "run-456",
